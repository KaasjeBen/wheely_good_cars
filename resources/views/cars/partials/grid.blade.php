...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
    ...
    @endphp
    <a href="{{ route('cars.show', $car) }}" class="group relative overflow-hidden rounded-lg border border-stone-200 bg-white p-5 flex flex-col gap-4 shadow-sm hover:-translate-y-0.5 hover:shadow-md transition {{ $isFeatured ? 'lg:col-span-2 lg:row-span-2' : '' }}">
        <div class="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r {{ $palette['stripe'] }}"></div>
        <div class="relative flex items-start justify-between gap-3">
            <div>
                <p class="text-sm text-stone-600">{{ $car->make }}</p>
                <h2 class="text-xl md:text-2xl font-semibold leading-snug text-stone-900">{{ $car->display_title }}</h2>
                <p class="text-sm text-stone-600">{{ $car->year ?? 'Onbekend bouwjaar' }} • {{ number_format($car->mileage, 0, ',', '.') }} km</p>
            </div>
            <span class="px-3 py-2 rounded-md {{ $palette['pill']['bg'] }} {{ $palette['pill']['text'] }} {{ $palette['pill']['border'] }} border text-sm font-semibold shadow-[0_1px_2px_rgba(0,0,0,0.05)]">€ {{ number_format($car->price, 0, ',', '.') }}</span>
        </div>
        <div class="relative h-40 md:h-44 {{ $isFeatured ? 'lg:h-64' : '' }} rounded-md border border-stone-200 overflow-hidden flex items-center justify-center bg-gradient-to-br {{ $palette['placeholder'] }}">
            ...
            <span class="relative text-4xl font-black text-stone-300">WG</span>
        </div>
        <div class="flex flex-wrap gap-2">
            @foreach ($car->tags->take(4) as $tag)
            <span class="px-3 py-1 rounded-full {{ $palette['tag']['bg'] }} {{ $palette['tag']['border'] }} text-xs {{ $palette['tag']['text'] }}">{{ $tag->name }}</span>
            @endforeach
            @if ($car->tags->count() > 4)
            <span class="px-3 py-1 rounded-full {{ $palette['tag']['bg'] }} {{ $palette['tag']['border'] }} text-xs {{ $palette['tag']['text'] }}">+{{ $car->tags->count() - 4 }}</span>
            @endif
        </div>
        <div class="flex items-center justify-between text-sm text-stone-700">
            <span>Bekeken {{ $car->views }}x</span>
            <span class="group-hover:text-amber-700 transition-colors">Bekijk details →</span>
        </div>
    </a>
    @empty
